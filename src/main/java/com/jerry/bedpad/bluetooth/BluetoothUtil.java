package com.jerry.bedpad.bluetooth;
...
public class BluetoothUtil extends BluetoothProtocol {
    ...
    public interface OnDeviceFound {
        void onLeScan(TemperatureDevice device);
    }
...
//                Log.i(TAG, str);
                if (mOnDeviceFound != null) {
                    ...
                    device.setReceive(getUsefulValue(receiveBytes));
                    device.setRssi(i);
                    device.setMacType(bluetoothDevice.getType());
     ...
     * @return
     */
    private byte[] getUsefulValue(byte[] receiveBytes) {
        int count = 0;
        for (int i = receiveBytes.length - 1; i >= 0; i--) {
            if (receiveBytes[i] == 0x00) {
                count++;
            }
        }
        byte[] value = new byte[receiveBytes.length - count];
        for (int i = 0; i < value.length - 1; i++) {
            value[i] = receiveBytes[i];
        }
        return value;
    }
...
//        Log.i(TAG, "停止扫描扫描蓝牙设备");
     ...
     */
    public static boolean checkBluetoothEnvironment(Context context) {
        if (!BluetoothEnvironment.isDeviceSupported(context)) {
            T.showShort(context, "设备不支持蓝牙4.0");
            return false;
        }
        BluetoothUtil.getInstance().initBluetooth(context, null);
        return true;
    }
